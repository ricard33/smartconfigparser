...
class SmartConfigParserTestCases(unittest.TestCase):
    def setUp(self):
        self.config = Config()
        self.config.read(os.path.join(test_path, 'config.ini'))
        self.unknown_section = str(uuid.uuid4())

    def test_get_default_if_section_does_not_exist(self):
        self.assertEqual('John', self.config.get(self.unknown_section, 'user', 'John'))
    ...
    def test_set_default_value(self):
        self.config.set(self.unknown_section, 'user', 'John DOE')
        ...
        self.assertTrue("user = John DOE" in content)
